...
{
    unsigned char state;
    int alllen;  ///< 所有字节长度，包含插入的转义字节
    unsigned char linktrs;
    int datalen; ///< 除去转义字节后的长度，用于计算是否收齐所有字节（因协议len长度不包含插入的转义字节）
...
unsigned short crc8408(int length, unsigned char *address)
{   
    int i,j;
    unsigned short crc;
    crc=0xffff;
...
unsigned char bcc_checksum(unsigned char* data, int len)
{
...
static int strim(unsigned char* des,unsigned char*src, int srclen)
{
...
static int unstrim(unsigned char*des,unsigned char*src,int srclen)
{
    // 在数据区域中0xF5后面插入0x55
    int i;
    int deslen = 0;
...
inline static unsigned char isUartMsgLenValid(int len)
{
...
/**
   ...
   @param des 接收数据的指针
     @retval 数据长度
*/
int getUserUartLinkMsg(unsigned char* des)
{
...
/**
   ...
   @param len 本次处理的数据长度
     @retval none
*/
bool UserUartLinkUnpack(unsigned char* data, int len)
{
    int i;
    unsigned char check;
    unsigned char valid;
    ...
    for(i = 0;i < len;i++)
    {
        ...
        case UART_LINK_LOAD:
            ...
            if(sUserUartLinkContext.datalen >= (sUserUartLinkContext.data[UART_LINK_LEN]+UART_LINK_LEN+1) )
            {
                ...
                {
                    qDebug()<<"Line:"<<__LINE__<<"收到的CRC:"<<crc;
                    qDebug()<<"Line:"<<__LINE__<<"计算的CRC"<<crccalc;
                    exitUartLinkUnpack();
                    return false;
                }
...
/**
   ...
   @param check 校验类型(0:crc8408,else:bcc)
     @retval 打包后的字节长度
*/
int UserUartLinkPack(unsigned char* des,unsigned char*src,int srclen,unsigned char check)
{
